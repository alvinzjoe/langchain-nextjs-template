  ...
  return (
    ...
    >
      <div className={`text-right ${expanded ? "w-full" : ""}`} onClick={(e) => setExpanded(!expanded)}>
        { action && action.tool && <code className="mr-2 bg-slate-600 px-2 py-1 rounded hover:text-blue-600">
          🛠️ <b>{action?.tool}</b>
        </code> }
      ...
      <div className={`overflow-hidden max-h-[0px] transition-[max-height] ease-in-out ${expanded ? "max-h-[360px]" : ""}`}>
        ...
        { action && action.toolInput && <code className={`opacity-0 max-h-[100px] overflow-auto transition ease-in-out delay-150 ${expanded ? "opacity-100" : ""}`}>
            Tool Input:
            <br></br>
            <br></br>
