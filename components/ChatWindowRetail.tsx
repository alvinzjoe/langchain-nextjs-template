...
export function ChatWindowRetail(props: {
  ...
  useEffect(() => {
    const input = "Hi there! I\'m RetailMate, your personal shopping assistant  🎉";
    const messagesWithUserReply = messages.concat({ id: messages.length.toString(), content: input, role: "assistant" });
    setMessages(messagesWithUserReply);
  }, [])
  ...
  return (
    <div className={`relative flex flex-col items-center p-4 md:p-8 px-0 md:px-8 rounded grow overflow-hidden ${(messages.length > 0 ? "border-0" : "")}`}>
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col absolute bottom-0 p-0 lg:p-6 bg-[#131318]">
        <div className="flex">
          {intemediateStepsToggle}
        </div>
        <div className="flex w-full mt-4">
          <input
            className="grow mr-4 md:mr-8 p-4 rounded"
            value={input}
            placeholder={placeholder ?? "What's it like to be a pirate?"}
            onChange={handleInputChange}
          />
          <button type="submit" className="px-2 py-4 bg-[#5c59ff] rounded w-20 lg:w-28">
